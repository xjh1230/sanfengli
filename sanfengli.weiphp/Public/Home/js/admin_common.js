...
$(function() {
	...
	*/
    ...
    $('.list_copy_link').each(function(index, element) {
        var isShopVote = false;
        $('.list_copy_link').parents().find('.current').each(function(i, v) {
            $a = $(v).find('a');
            var href = $a.attr('href');
            if (href.indexOf('Vote/ShopVote/lists') > 0) {
                isShopVote = true;
                return false;
            }
        });
        if (isShopVote) {
            var id = $(this).attr('id').replace('$(this).parent()', '');
            $(this).data('clipboard-text', 'http://localhost:89/home/votelist.aspx?vote_id=' + id)
        }
...
function initUploadImg(opts) {
    $(".upload-img-view").dragsort({
        ...
        dragEnd: function() { $(".upload-pre-item22").attr('style', '') }
    });

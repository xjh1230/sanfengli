...
class ShopVoteController extends ManageBaseController {
	...
	function option_lists() {
		...
		$list_data = $this->_get_model_list ( $model,0,'opt_count desc,number asc' );
		$this->assign ( $list_data );
		$this->display ( 'lists' );
	}
	function option_add() {
...
// 			$_POST ['number'] = $num;
	...
	function preview() {
		...
		// 		'vote_id' => $id 
		// ) );
		$tmp="http://sanfengli.koalajoy.com/home/votelist.aspx?vote_id=";
		$url=$tmp . $id;
		$this->assign ( 'url', $url );
	...
	// 修改审核
	function option_validate($id) {//
		$model = $this->getModel ( 'shop_vote_option' );
		$data = M ( get_table_name ( $model ['id'] ) )->find ( $id );
		if($data['option_status']==1){
			$data ['option_status']=0;
		}else{
			$data ['option_status']=1;
		}
		$db=M('shop_vote_option');
		$result = $db->save ($data); 
		if ($result) {
			$this->success ( '修改成功' );
		} else {

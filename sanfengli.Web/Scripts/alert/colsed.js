...
function ShowTxt(_PromptText, _BtnOkText, _BtnOkEvent, _BtnCancelText, _BtnCancelEvent, _InputText, _NotNullTitle) {
    ...
    HideTxt();//删除原来浮层
    //添加弹出框；
    $("html").append(_appHTML);

    //绑定事件
    $(".span_btn a").each(function (index) {
        $(this).attr("id", $(this).attr("class") + "_" + index);
        var domElement = document.getElementById($(this).attr("id"));
        if (window.addEventListener) { _bind(domElement, "click", HideTxt); }
        var _a_classname = $(this).attr("class");
        //确定按钮添加事件
    ...
    //关闭；
    $(".cd_btn").click(function () { HideTxt() }); //给那个右上角的关闭按钮加事件
}
